...
export default function Chat() {
  ...
  const chatStart = async function(){
    try{    
      ...
      console.log(json);
      if(json.data){
        let target = JSON.parse(json.data);
        console.log(target.candidates[0].content.parts[0]);
        const mdText = target.candidates[0].content.parts[0].text
        const htm = marked.parse(mdText);
        //console.log("part=", target.candidates[0].content.parts[0]);
